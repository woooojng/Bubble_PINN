...
def coefficientMvBdry(net, x, y, t):
    ...
    gradp_norm_MvBdry = torch.cdist(gradp_MvBdry, torch.zeros(1, 2).to(device), p=1)
    ...
    MvBdryCoefficient = 1/((gradp_norm_MvBdry)+ torch.ones_like(gradp_norm_MvBdry))
...
def lossMvBdry(net, x, y, t):
    ...
    small = .01 #Keep in mind the size of this
    ...
    jump_u1 = torch.cdist(u1_inner-u1_outer, torch.zeros(1, 1).to(device), p=2)
    jump_u2 = torch.cdist(u2_inner-u2_outer, torch.zeros(1, 1).to(device), p=2)
