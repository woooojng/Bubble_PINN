...
def rise_vel_max(net, t):
    ...
    u2_unit = torch.where(rho ==1000, 0, 1)
    u2 = torch.where(rho ==1000, 0, u2)

    #Compute the denomenator =int_{Omega2} 1 dx
    omega2_area = 1/Domain_collocation *(torch.sum(u2_unit))
    
    
    #Compute the nominator =int_{Omega2} x dx
    omega2_exp = 1/Domain_collocation *torch.sum(u2)
